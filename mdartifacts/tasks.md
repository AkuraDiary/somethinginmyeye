...
- [X] **Ekspansi Fitur Preprocessing (Python):** Update file `preprocess.py` (atau script pemrosesan) untuk menghitung 5 fitur baru menggunakan Pandas: `Delta_X`, `Delta_Y`, `Acceleration`, dan `Jerk`. Ubah `FEATURES = 3` menjadi `FEATURES = 8` di `config.py`.
...
- [X] **Upgrade ke Bidirectional LSTM:** Setelah 200 data terkumpul, ganti layer `Conv1D` di Jupyter Notebook dengan layer `Bidirectional(LSTM)` agar AI memiliki "ingatan" jangka panjang terhadap ritme menulis user.
